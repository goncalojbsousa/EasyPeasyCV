...
export function getSectionOrder(
  ...
  const customOrder = (customSections || []).map((cs) => `custom_${cs.id}` as SectionKey);
...
export function renderSummarySection(
  ...
  const label = lang === 'en' ? 'SUMMARY' : lang === 'es' ? 'RESUMEN' : 'RESUMO';
...
export function renderExperienceSection(
  ...
  const label = lang === 'en' ? 'EXPERIENCE' : lang === 'es' ? 'EXPERIENCIA' : 'EXPERIÊNCIA';
  ...
  const itemsContent = (
    <>
      {experiences.map((exp, idx) => {
          ...
          <View key={idx} style={styles.expItem}>
            <View style={styles.expHeaderRow}>
              ...
              <View style={styles.expRight}>
                <Text>{formatDateRange(exp.startMonth, exp.startYear, exp.endMonth, exp.endYear, exp.current, lang, settings?.sections?.dateFormat)}</Text>
              </View>
            </View>
            {exp.activities && <Text style={styles.activitiesText}>{exp.activities}</Text>}
            {exp.results && (
        ...
        return ItemWrapper ? <ItemWrapper key={idx}>{content}</ItemWrapper> : content;
      })}
    </>
  );
  
  if (SectionWrapper) {
    return <SectionWrapper sectionKey="professional_experience" label={label}>{itemsContent}</SectionWrapper>;
  }
...
export function renderEducationSection(
  ...
  const label = lang === 'en' ? 'EDUCATION' : lang === 'es' ? 'EDUCACIÓN' : 'EDUCAÇÃO';
  
  const itemsContent = (
    <>
      {education.map((edu, idx) => {
        const typeLabel = translateLabel(edu.type, lang === 'pt' ? 'pt' : lang === 'es' ? 'es' : 'en');
        const statusLabel = translateLabel(edu.status, lang === 'pt' ? 'pt' : lang === 'es' ? 'es' : 'en');
              ...
              <View style={styles.expLeft}>
                ...
                {(edu.institution || meta) && (
                  <Text style={{ ...styles.company, marginBottom: 0 }}>
                    {edu.institution}
                    {meta && <Text style={{ fontSize: 9, color: '#000000', marginTop: 0 }}>{'  |  '}{meta}</Text>}
                  </Text>
                )}
              </View>
              <View style={styles.expRight}>
                <Text>{formatDateRange(edu.startMonth, edu.startYear, edu.endMonth, edu.endYear, edu.current, lang, settings?.sections?.dateFormat)}</Text>
              </View>
            </View>
            {edu.description && <Text style={styles.activitiesText}>{edu.description}</Text>}
            {edu.achievements && (
              <View style={{ marginTop: 2 }}>
  ...
  if (SectionWrapper) {
    return <SectionWrapper sectionKey="academic_education" label={label}>{itemsContent}</SectionWrapper>;
  }
...
export function renderSkillsSection(
  ...
  const label = lang === 'en' ? 'SKILLS' : lang === 'es' ? 'HABILIDADES' : 'COMPETÊNCIAS';
      ...
      {CustomRender ? (
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', flexWrap: 'wrap' }}>
          {languages.map((langItem, li) => {
            const levelLabel = translateLanguageLevel(langItem.level, lang === 'pt' ? 'pt' : lang === 'es' ? 'es' : 'en');
            return (
...
export function renderCertificationsSection(
  ...
  const label = lang === 'en' ? 'CERTIFICATIONS' : lang === 'es' ? 'CERTIFICACIONES' : 'CERTIFICAÇÕES';
...
export function renderProjectsSection(
  ...
  const label = lang === 'en' ? 'PROJECTS' : lang === 'es' ? 'PROYECTOS' : 'PROJETOS';
  ...
  return (
    <View style={styles.section} key="projects">
      ...
      {projects.map((proj, i) => (
        <View key={i} style={{ marginBottom: 6 }}>
          <Text style={styles.jobRole}>
            {proj.name} {proj.year ? <Text style={{ fontSize: 10, fontStyle: 'italic' }}>{proj.year}</Text> : null}
          </Text>
          {proj.tech && <Text style={{ fontSize: 10, color: '#000000' }}>{proj.tech}</Text>}
          {proj.description && <Text style={styles.activitiesText}>{proj.description}</Text>}
          {proj.impact && (
            <View style={{ marginTop: 2 }}>
  ...
  const itemsContent = (
    <>
      {volunteers.map((vol, i) => {
        const content = (
          <View key={i} style={{ marginBottom: 6 }}>
            <View style={styles.expHeaderRow}>
              ...
              <View style={styles.expRight}>
                <Text>{formatDateRange(vol.startMonth, vol.startYear, vol.endMonth, vol.endYear, vol.current, lang, settings?.sections?.dateFormat)}</Text>
              </View>
            </View>
            {vol.description && <Text style={styles.activitiesText}>{vol.description}</Text>}
            {vol.impact && (
              <View style={{ marginTop: 2 }}>
        ...
        return ItemWrapper ? <ItemWrapper key={i}>{content}</ItemWrapper> : content;
      })}
    </>
  );
  
  if (SectionWrapper) {
    return <SectionWrapper sectionKey="volunteer" label={label}>{itemsContent}</SectionWrapper>;
  }
...
export function renderCustomSection(
  ...
  const meaningfulFields = (section.fields || []).filter(
    (f) => f.label || f.value || f.bullets || f.subtitle || f.startYear || f.endYear || f.startMonth || f.endMonth || f.current
  );
  
  if (!section.title && meaningfulFields.length === 0) return null;
  
  const label = section.title || (lang === 'en' ? 'CUSTOM SECTION' : lang === 'es' ? 'SECCIÓN PERSONALIZADA' : 'SECÇÃO PERSONALIZADA');
  ...
  const itemsContent = (
    <>
      {meaningfulFields.map((field, idx) => {
        const content = (
          <View key={field.id || idx} style={{ marginBottom: 8 }}>
            {(field.label || field.subtitle || field.startYear || field.endYear || field.startMonth || field.endMonth || field.current) && (
              <View style={styles.expHeaderRow}>
                <View style={styles.expLeft}>
                  {field.label && <Text style={styles.jobRole}>{field.label}</Text>}
                  {field.subtitle && <Text style={styles.company}>{field.subtitle}</Text>}
                </View>
                <View style={styles.expRight}>
                  <Text>{formatDateRange(field.startMonth, field.startYear, field.endMonth, field.endYear, field.current, lang, settings?.sections?.dateFormat)}</Text>
                </View>
              </View>
            )}
            {field.value && (
              <Text style={field.centerValue ? { ...styles.activitiesText, textAlign: 'center' } : styles.activitiesText}>
                {field.value}
              </Text>
            )}
            {field.bullets && (
        ...
        return ItemWrapper ? <ItemWrapper key={field.id || idx}>{content}</ItemWrapper> : content;
      })}
    </>
  );
  
  if (SectionWrapper) {
    return <SectionWrapper sectionKey={sectionKey} label={label}>{itemsContent}</SectionWrapper>;
  }
