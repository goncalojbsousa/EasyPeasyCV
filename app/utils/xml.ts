...
export function cvDataToXml(data: CvData): string {
	const linksXml = (data.links || [])
		.map(
			(l: Link) =>
				`<link>${el("type", l.type)}${el("value", l.value)}${el("customName", l.customName || "")}</link>`,
		)
		.join("");
...
export function xmlToCvData(xml: string): CvData {
	...
	const personalInfo = {
		...
		countryCode:
			textContent(personalInfoEl, "countryCode") || "Portugal (+351)",
		phone: textContent(personalInfoEl, "phone"),
	};

	const links: Link[] = Array.from(
		cvEl.getElementsByTagName("links")[0]?.getElementsByTagName("link") || [],
	).map((ln) => ({
	...
	const experiences: Experience[] = Array.from(
		cvEl
			.getElementsByTagName("experiences")[0]
	...
	const education: Education[] = Array.from(
		cvEl
			.getElementsByTagName("educations")[0]
	...
	const certifications: Certification[] = Array.from(
		cvEl
			.getElementsByTagName("certifications")[0]
			?.getElementsByTagName("certification") || [],
	).map((ce) => ({
	...
	const projects: Project[] = Array.from(
		cvEl.getElementsByTagName("projects")[0]?.getElementsByTagName("project") ||
			[],
	).map((pr) => ({
	...
	const volunteers: Volunteer[] = Array.from(
		cvEl
			.getElementsByTagName("volunteers")[0]
			?.getElementsByTagName("volunteer") || [],
	).map((vo) => ({
	...
	const customSections: CustomSection[] = Array.from(
		cvEl
			.getElementsByTagName("customSections")[0]
			?.getElementsByTagName("customSection") || [],
	).map((sec) => {
	...
	const sectionOrder = Array.from(
		cvEl
			.getElementsByTagName("sectionOrder")[0]
