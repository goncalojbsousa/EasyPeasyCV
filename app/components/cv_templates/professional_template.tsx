...
const buildStyles = (settings?: CvRenderSettings, color: CvColor = 'blue') => {
  ...
  const specificStyles = StyleSheet.create({
    ...
    contactItem: { marginHorizontal: 6, flexDirection: 'row', alignItems: 'center' },
    ...
    photo: { width: photoSize.width, height: photoSize.height, borderRadius: photoBorderRadius, marginLeft: 10 },
    photoImage: { width: photoSize.width, height: photoSize.height, objectFit: 'cover', borderRadius: photoBorderRadius },
  });
                ...
                <View style={styles.contactRow}>
                  {contactItems.map((item, i) => (
                    <React.Fragment key={i}>
                      ...
                      {i < contactItems.length - 1 ? <Text style={styles.contactSeparator}>|</Text> : null}
                    </React.Fragment>
                  ))}
                </View>
              )}
