...
export function BottomActionBar({
  ...
  const [openMenu, setOpenMenu] = useState<null | 'lang' | 'cvType' | 'data' | 'layout'>(null);
        ...
        <div className="inline-flex max-w-[calc(100vw-2rem)] rounded-xl border border-gray-200/80 dark:border-zinc-700/60 shadow-xl ring-1 ring-black/10 bg-white/70 dark:bg-zinc-900/60 backdrop-blur supports-[backdrop-filter]:bg-white/60 supports-[backdrop-filter]:dark:bg-zinc-900/50">
          <div className="px-2.5 py-2">
            ...
            <div className="relative shrink-0 overflow-visible" ref={cvTypeRef}>
              <button
                ref={cvTypeBtnRef}
                onClick={() => setOpenMenu((prev) => (prev === 'cvType' ? null : 'cvType'))}
              ...
              >
                ...
                <ChevronDown className={`w-4 h-4 transition-transform ${openMenu === 'cvType' ? 'rotate-180' : ''}`} />
              </button>
              {openMenu === 'cvType' && cvTypePos && createPortal(
                <div ref={cvTypePortalRef} className="z-[70] w-[260px] max-h-[60vh] overflow-auto bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-gray-200 dark:border-zinc-700 py-2"
                  ...
                  <div className="py-1">
                    {cvTypes.map((type) => (
                      <button
                        key={type}
                        onClick={() => { setCVType(type); setOpenMenu(null); }}
            ...
            <div className="relative shrink-0 overflow-visible">
              <button
                ref={layoutBtnRef}
                onClick={(e) => { e.stopPropagation(); setOpenMenu((prev) => (prev === 'layout' ? null : 'layout')); }}
                className="flex h-9 items-center gap-2 px-3 rounded-md border border-gray-300/60 dark:border-zinc-600/60 bg-white/80 dark:bg-zinc-800/80 text-[13px] text-gray-900 dark:text-gray-100 hover:bg-white dark:hover:bg-zinc-700 shadow-sm"
                title={t('layout.menu.title')}
              >
                <Grid2x2 className="w-4 h-4" />
                <span className="font-medium">{t('layout.menu.title')}</span>
                <ChevronDown className={`w-4 h-4 transition-transform ${openMenu === 'layout' ? 'rotate-180' : ''}`} />
              </button>
              {openMenu === 'layout' && layoutPos && createPortal(
            ...
            <div className="relative shrink-0 overflow-visible" ref={langRef}>
              ...
              >
                ...
                <ChevronDown className={`w-4 h-4 transition-transform ${openMenu === 'lang' ? 'rotate-180' : ''}`} />
              </button>
              {openMenu === 'lang' && langPos && createPortal(
                <div ref={langPortalRef} className="z-[70] w-[260px] max-h-[60vh] overflow-auto bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-gray-200 dark:border-zinc-700 py-2"
                  onMouseDown={(e) => e.stopPropagation()}
                  ...
                  <div className="py-1">
                    {languageOptions.map((opt) => (
                      <PdfDownloadButtonWithValidation key={opt.code} lang={opt.code}>
            ...
            <div className="relative shrink-0 overflow-visible" ref={dataRef}>
              ...
              <button
                ref={dataBtnRef}
                onClick={() => setOpenMenu((prev) => (prev === 'data' ? null : 'data'))}
                className="flex h-9 items-center gap-2 px-3 rounded-md border border-gray-300/60 dark:border-zinc-600/60 bg-white/80 dark:bg-zinc-800/80 text-[13px] text-gray-900 dark:text-gray-100 hover:bg-white dark:hover:bg-zinc-700 shadow-sm"
                title={t('data.xml.title')}
              >
                <Database className="w-4 h-4" />
                <span className="font-medium">{t('data.xml.title')}</span>
                <ChevronDown className={`w-4 h-4 transition-transform ${openMenu === 'data' ? 'rotate-180' : ''}`} />
              </button>
              {openMenu === 'data' && dataPos && createPortal(
                <div ref={dataPortalRef} className="z-[70] w-[260px] max-h-[60vh] overflow-auto bg-white dark:bg-zinc-800 rounded-xl shadow-xl border border-gray-200 dark:border-zinc-700 py-2"
                  onMouseDown={(e) => e.stopPropagation()}
                  ...
                  <button type="button" className="w-full text-left px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-sky-50 dark:hover:bg-sky-900/20 transition-colors duration-200 flex items-center gap-2" onClick={() => { setOpenMenu(null); onExportXml(); }}>
