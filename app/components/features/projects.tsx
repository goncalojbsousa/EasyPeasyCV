	...
	return (
		<form className="space-y-8 flex flex-col items-center">
			...
			>
				...
				{projects.length === 0 && <EmptyState message={t("empty.project")} />}

				{/* Render each project entry using SortableList */}
				<SortableList
					length={projects.length}
					onReorder={(from, to) => onReorderProjects?.(from, to)}
					renderItem={(idx) => {
								...
								<div className="p-4">
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4">
										<FormField label={t("field.project.name")}>
											<input
												...
												value={proj.name}
												onChange={(e) =>
													onProjectChange(idx, "name", e.target.value)
												}
											/>
										</FormField>
										<FormField label={t("field.year")}>
											<input
												...
												value={proj.year}
												onChange={(e) =>
													onProjectChange(idx, "year", e.target.value)
												}
											/>
										</FormField>
									</div>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4">
										...
										<FormField label={t("field.project.link")}>
											<input
												...
												value={proj.link}
												onChange={(e) =>
													onProjectChange(idx, "link", e.target.value)
												}
											/>
										</FormField>
									</div>
									...
									<div className="grid grid-cols-1 gap-4 sm:gap-6 mb-4">
										<FormField label={t(`cvType.field.technologies`)}>
											<input
												...
												value={proj.tech}
												onChange={(e) =>
													onProjectChange(idx, "tech", e.target.value)
												}
											/>
										</FormField>
									</div>
									...
									<div className="mb-4">
										<FormField label={t("field.description")}>
											<AutoResizeTextarea
												className="w-full p-2 border border-gray-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-800 shadow-sm focus:outline-none focus:ring-1 focus:ring-sky-400 focus:border-sky-400 transition-all text-sm text-gray-900 dark:text-gray-100"
												placeholder={t(
													`cvType.placeholder.project.description`,
												)}
									...
									>
										<AutoResizeTextarea
											...
											value={proj.impact}
											onChange={(e) =>
												onProjectChange(idx, "impact", e.target.value)
											}
											minHeight={80}
										/>
									</FormField>
