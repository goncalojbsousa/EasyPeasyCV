...
import type { Volunteer } from "../../types/cv";
import {
		...
		<form className="space-y-8 flex flex-col items-center">
			...
			>
				...
				<SortableList
					...
					onReorder={(from, to) => onReorderVolunteers?.(from, to)}
					renderItem={(idx) => {
								...
								<div className="p-4">
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4">
										...
										<FormField label={t("field.role")}>
											<input
												...
												value={vol.role}
												onChange={(e) =>
													onVolunteerChange(idx, "role", e.target.value)
												}
											/>
										</FormField>
									</div>
									...
									<div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-4">
										...
										<FormField label={t("field.start.year")}>
											<input
												...
												value={vol.startYear}
												onChange={(e) =>
													onVolunteerChange(idx, "startYear", e.target.value)
												}
											/>
										</FormField>
											...
											<>
												...
												<FormField label={t("field.end.year")}>
													<input
														...
														value={vol.endYear}
														onChange={(e) =>
															onVolunteerChange(idx, "endYear", e.target.value)
														}
													/>
												</FormField>
											</>
									...
									<div className="flex items-center gap-2 mb-4">
										<input
											...
											checked={vol.current}
											onChange={(e) =>
												onVolunteerChange(idx, "current", e.target.checked)
											}
											id={`volunteer-current-${idx}`}
											className="mr-2"
										/>
										<label
											htmlFor={`volunteer-current-${idx}`}
									...
									>
										<AutoResizeTextarea
											...
											value={vol.impact}
											onChange={(e) =>
												onVolunteerChange(idx, "impact", e.target.value)
											}
											minHeight={80}
										/>
									</FormField>
