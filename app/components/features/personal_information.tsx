				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4">
					<FormField label={t("field.full.name")}>
						<input
							...
							data-error={
								showValidationErrors && validationErrors.name ? "true" : "false"
							}
						/>
					</FormField>
					<FormField label={t(`cvType.field.desired.role`)}>
						<input
							...
							data-error={
								showValidationErrors && validationErrors.desiredRole
									? "true"
									: "false"
							}
						/>
						...
						<div className="mb-4 space-y-2">
							<SortableList
								...
								renderItem={(idx) => {
									...
									const linkType =
										LINK_TYPES.find((t) => t.label === link.type) ||
										LINK_TYPES[0];
									const displayValue =
										linkType.prefix &&
										link.value.startsWith(`https://www.${linkType.prefix}`)
											? link.value.replace(`https://www.${linkType.prefix}`, "")
											: linkType.prefix &&
													link.value.startsWith(`https://${linkType.prefix}`)
												? link.value.replace(`https://${linkType.prefix}`, "")
												: linkType.prefix &&
														link.value.startsWith(linkType.prefix)
